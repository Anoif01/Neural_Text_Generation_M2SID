...
class CopyAttention(object):
    ...
    def __init__(self, n, d, g, nhu, nW, nF, nQ,
        ...
        with tf.name_scope('embeddings'):
            # Word embeddings
            embed = get_hpca_embeddings('/content/drive/MyDrive/GM5/DEEP/Paper/NeuralTextGeneration-master/data/embeddings', d, nW)
    ...
    def _generate_x_ct(self, context, zp, zm, gf, gw, batch_size):
        ...
        #### TEST 
        x_ct = tf.concat((ct, zp_fin, zm_fin, gf_fin, gw_fin), 1)
        return x_ct
    ...
    def training(self, loss):
        ...
        print('ADAGRAD op')
        # optimizer = tf.train.AdamOptimizer(learning_rate=self._lr)#, beta1=0.9, beta2=0.99)
        optimizer = tf.train.AdagradOptimizer(learning_rate=self._lr)
        # optimizer = tf.train.RMSPropOptimizer(learning_rate=self._lr)#, decay=0.9)
